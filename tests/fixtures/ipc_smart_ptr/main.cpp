// Proxy/stub pair reached through an undeclared smart-pointer wrapper (#86).
...
// The wrapper's body is not in the tree: `->` resolves on its argument
// class, and the proxy method it lands on must still bridge to its handler.
int CallThroughWrapper(sptr<IFooProxy> p) { return p->GetInfo(7); }
int CallThroughWrapperField(OHOS::sptr<IFooProxy> p) { return p->SetInfo(1, 2); }
