...
files and loads them as parsing needs them. Parsing workers run at most four
units per worker ahead of the ordered merge, which limits queued IR without
making a batch wait for its slowest unit. Temporary files are cleaned
