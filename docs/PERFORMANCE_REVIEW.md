...
- Replaces fixed parse batches with a window: workers take units in order
  and run at most four units per worker ahead of the ordered merge. Worker
  count and merge order are unchanged, the pending unit count is bounded by
  the window, and a slow unit holds up the merge rather than the workers.
...
The patch adds no persistent storage, but these single-process measurements
do not establish whether the RSS difference comes from scheduling, allocation
lifetimes, or system memory pressure. More repetitions would be needed to
