...
- **Derived default**: `800 000 + 6 × (PAG constraint count)` pops (`default_pops_budget`). The flat 800 000 floor is the historical default and covers the eval corpora (the largest, HDF, needs ~42k pops). The linear term handles larger trees: `ability_ability_runtime` (257 K constraints) *converges* at 1.68 M pops — half the derived budget (2.34 M) — whereas the old flat 800 000 stopped it at a partial result with ~40% of indirect edges missing.
