...
/// Units a worker may take ahead of the merge, per worker. This bounds the
/// IR held before merging (at most `jobs * INDEX_WINDOW_PER_WORKER` units
/// indexed or in flight) without making anyone wait for a batch: a slow unit
/// holds up the merge, not the other workers.
...
/// worker that is `INDEX_WINDOW_PER_WORKER * jobs` units ahead of the merge
/// waits for it, so the pending IR is bounded by the window rather than by
/// the corpus, and the merge order is that of `items` regardless of which
/// unit finishes first.
fn index_in_window<T: Send>(
    ...
    let jobs = jobs.max(1);
    let window = jobs.saturating_mul(INDEX_WINDOW_PER_WORKER);
    ...
    let (index, state, changed) = (&index, &state, &changed);
    ...
    rayon::scope(|scope| {
        for _ in 0..jobs {
            scope.spawn(move |_| loop {
                let i = {
                    let mut st = lock();
                    while st.next >= st.merged + window && st.next < items.len() {
                        st = changed.wait(st).unwrap_or_else(|e| e.into_inner());
                    }
                    if st.next >= items.len() {
                        return;
                    }
                    st.next += 1;
                    st.next - 1
        ...
        pool.install(|| {
            index_in_window(
                ...
                |path| {
                    ...
                    assert!(
                        i < merged_count.load(Ordering::SeqCst) + jobs * INDEX_WINDOW_PER_WORKER
                    );
