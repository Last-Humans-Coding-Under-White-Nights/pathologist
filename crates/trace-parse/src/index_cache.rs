...
impl SpilledSource {
    fn store(src: &PreprocessedSource) -> std::io::Result<Self> {
            ...
            .tempfile()?;
        {
            ...
            let mut entries = Vec::with_capacity(src.line_map.entries.len() * LINE_MAP_ENTRY_BYTES);
            for e in &src.line_map.entries {
                for value in [e.output_offset, e.file, e.line, e.col] {
    ...
    fn load(&self) -> std::io::Result<PreprocessedSource> {
        ...
        let mut bytes = vec![0; self.entry_count * LINE_MAP_ENTRY_BYTES];
...
impl IndexSourceCache {
    ...
    pub(crate) fn check_load_errors(&self) -> Result<(), String> {
        let errors = self.load_errors.lock().map_err(|e| e.to_string())?;
        if errors.is_empty() {
            Ok(())
        } else {
